...
function isRetryableError(error) {
  ...
  return error?.code === "ECONNABORTED" || error?.code === "ETIMEDOUT" || error?.code === "ECONNRESET" || error?.code === "ENOTFOUND" || error?.code === "EAI_AGAIN" || error?.code === "UND_ERR_CONNECT_TIMEOUT" || error?.code === "ABORT_ERR" || error?.name === "TimeoutError" || error?.isAxiosError === true && !error.response;
}
